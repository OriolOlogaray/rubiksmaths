...
for move in scramble:
    ...
    if move == "B2":
        b(up, right, down, left, back)
        b(up, right, down, left, back)
print("Si vols la solució òptima, prem 1.")
print("Si vols la solució utilitzant el mètode CFOP, prem 2.")
method = str(input())
if method == "1":
    formatted_cube = ""
    ...
    if len(scramble) <= 5:
        ...
        while i < len(scramble) - 1:
            ...
            if item[:1] == next_item[:1]:
                ...
                final_solution += item + " "
                i += 1
        final_solution += scramble[-1] + " "
        fscramble = final_solution.split()
        final_solution = ""
        i = 0
        while i < len(fscramble) - 1:
            item = fscramble[i]
            next_item = fscramble[i + 1]
            if item[:1] == next_item[:1]:
                if len(item) == 1:
                    if len(next_item) == 1:
                        final_solution += item + "2 "
